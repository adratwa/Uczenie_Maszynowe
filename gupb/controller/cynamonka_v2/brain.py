...
class Map:
    ...
    def find_dangerous_tiles(self):
        ...
        for coords in self.terrain:
            ...
            if enemy_description is not None and enemy_description.controller_name != "Cynamonka2":
...
class MyKnowledge:
    ...
    def is_enemeny_in_attackable_area(self):
        ...
        for coords in attackable_area:
            if self.map.terrain[coords].character is not None and self.map.terrain[coords].character.controller_name != "Cynamonka2":
                return True
        return False
    ...
    def can_attack(self):
        ...
        for coords, description in self.map.terrain.items():
            if description.character and description.character.controller_name != "Cynamonka2" and coords in attackable_area:
