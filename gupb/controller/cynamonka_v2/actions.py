...
class Actions:
    ...
    def go_in_menhir_direction(self, menhir_position):
        action, path_to_menhir = self.path_finder.calculate_next_move(menhir_position)
    ...
    def go_to_specific_point(self, specific_point):
        action, path = self.path_finder.calculate_next_move(specific_point)
        print(f"path to specific point: {path}")
        if path and action:
            return action
        return self.go_randomly()
